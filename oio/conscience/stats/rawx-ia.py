...
class RawxIaStat(HttpStat):
    """Specialization of HttpStat for rawxia services"""
    ...
    def configure(self):
        super(RawxIaStat, self).configure()
    ...
    def get_stats(self):
        stats = super(RawxIaStat, self).get_stats()
        if not stats:
            return stats
        ...
        output = dict()
        for stat_key in RawxIaStat.rawxia_stat_keys:
